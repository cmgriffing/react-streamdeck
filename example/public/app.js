...
async function connected(jsn) {
  ...
  $SD.on("com.cmg.testing.action.willAppear", jsonObj =>
    action.onWillAppear(jsonObj)
  );
  $SD.on("com.cmg.testing.action.keyUp", jsonObj => {
    $SD.api.getSettings(context);
    $SD.api.sendToPropertyInspector(
      context,
      { testing: "testing" },
      "com.cmg.testing.action"
    );
    return action.onKeyUp(jsonObj);
  });
  $SD.on("com.cmg.testing.action.sendToPlugin", jsonObj =>
    action.onSendToPlugin(jsonObj)
  );
  $SD.on("com.cmg.testing.action.didReceiveSettings", jsonObj => {
  ...
  $SD.on("com.cmg.testing.action.propertyInspectorDidAppear", jsonObj => {
  ...
  $SD.on("com.cmg.testing.action.propertyInspectorDidDisappear", jsonObj => {
    console.log(
      "%c%s",
